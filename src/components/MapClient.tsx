            ...
            <div className="min-w-[180px] max-w-[240px]">
              {event.image_url && (
                <img
                  src={event.image_url}
                  alt={event.title}
                  className="w-full h-28 object-cover rounded mb-2 -mx-0"
                />
              )}
              <p className="font-semibold text-sm leading-snug">{event.title}</p>
              {event.location_name && (
                <p className="text-xs text-gray-500 mt-0.5">{event.location_name}</p>
              )}
              ...
              {event.ends_at && (
                <p className="text-xs text-gray-400">– {formatDateTime(event.ends_at)}</p>
              )}
              {event.url && (
                <a
                  href={event.url}
